# ADR 0002: 平台文生图复用 CowAgent Skill 内核
...
CowAgent 2.0.7 新增内置 `image-generation` Skill，封装多厂商文生图、图生图和多图融合能力。  
...
平台文生图能力统一复用 CowAgent 2.0.7 `skills/image-generation` 作为执行内核：
...
这样可以把平台配置、CowAgent Skill 内核和上游 2.0.7 后续修复收敛到一条路径，减少重复逻辑。  
