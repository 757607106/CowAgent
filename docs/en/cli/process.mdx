...
description: Manage CoreAgent process lifecycle with cow commands
---

Process management commands control the CoreAgent background process. These commands are only available in the terminal.

## start

Start the CoreAgent service. Runs as a background daemon by default and automatically tails logs.
...
Stop the running CoreAgent service.
...
Restart the CoreAgent service (stop then start).
...
Check CoreAgent service status, including process info, version, and current model/channel configuration.
