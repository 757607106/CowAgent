---
title: Web Console
description: Use CoreAgent through the web console
---

The Web Console is CoreAgent's default channel. It starts automatically after launch, allowing you to chat with the Agent through a browser and manage models, skills, memory, channels, and other configurations online.
...
```json
{
  "channel_type": "web",
  "web_port": 9899,
  "platform_database_url": "postgresql://cowplatform:prod-smoke-db-secret@127.0.0.1:55432/cowplatform"
}
```
...
| `platform_database_url` | PostgreSQL DSN for tenants, agents, sessions, memory indexes, jobs, and audit logs. Local source startup derives it from `.env.docker` and connects to Docker-mapped ports. | `postgresql://cowplatform:prod-smoke-db-secret@127.0.0.1:55432/cowplatform` |
...
The chat interface supports multi-session management. All session records are persistently stored in PostgreSQL:
