---
title: Web 控制台
description: 通过 Web 控制台使用 CoreAgent
---

Web 控制台是 CoreAgent 的默认通道，启动后会自动运行，通过浏览器即可与 Agent 对话，并支持在线管理模型、技能、记忆、通道等配置。
...
| `web_password` | 兼容旧版的控制台访问密码；当 `web_tenant_auth=false` 时生效 | `""` |
| `web_tenant_auth` | 启用租户注册/登录，并按当前租户隔离控制台数据 | `true` |
| `web_session_expire_days` | 登录会话有效天数 | `30` |
| `platform_database_url` | PostgreSQL 连接串，用于租户、Agent、会话、记忆索引、任务和审计等平台数据；本地源码启动从 `.env.docker` 派生并连接 Docker 映射端口 | `postgresql://cowplatform:prod-smoke-db-secret@127.0.0.1:55432/cowplatform` |
| `enable_thinking` | 是否启用深度思考，开启后 Web 端展示推理过程，关闭可加速响应 | `false` |

默认会进入租户注册/登录流程；注册租户后系统会自动创建该租户的 owner 用户和通用 Agent。登录状态默认保持 30 天，期间重启服务也无需重新登录。若需要旧版单密码模式，可将 `web_tenant_auth` 设为 `false` 并配置 `web_password`。
...
对话界面支持多会话（Session）管理，所有会话记录持久化存储在 PostgreSQL 中：
