---
title: プロセス管理
description: cow コマンドで CoreAgent プロセスのライフサイクルを管理
---

プロセス管理コマンドは CoreAgent バックグラウンドプロセスのライフサイクルを制御します。これらのコマンドはターミナルでのみ使用可能です。

## start

CoreAgent サービスを起動します。デフォルトではバックグラウンドデーモンとして実行され、自動的にログを表示します。
...
CoreAgent サービスを再起動します（停止してから起動）。
...
CoreAgent サービスの実行状態を確認します。プロセス情報、バージョン、現在のモデルとチャネルの設定を含みます。
