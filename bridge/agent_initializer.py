...
class AgentInitializer:
    ...
    def initialize_agent(self, session_id: Optional[str] = None) -> Agent:
        ...
        # 平台模式下优先使用运行时作用域里的工作区
        runtime_context = None
        agent_definition = None
        try:
            from cow_platform.runtime.scope import (
                get_current_agent_definition,
        ...
        if runtime_context is not None:
            workspace_root = str(runtime_context.workspace_path)
        else:
        ...
        if agent_definition is not None:
            knowledge_enabled = bool(agent_definition.knowledge_enabled)

        workspace_files = ensure_workspace(
            workspace_root,
        ...
        skill_manager = self._initialize_skill_manager(workspace_root, session_id, agent_definition=agent_definition)
        ...
        # 显式 Agent 的自定义提示词优先拼接在最前面
        if agent_definition and agent_definition.system_prompt.strip():
            custom_system_prompt = agent_definition.system_prompt.strip()
        ...
        if session_id:
            self._restore_conversation_history(agent, session_id, workspace_root)
    ...
    def _restore_conversation_history(self, agent, session_id: str, workspace_root: str) -> None:
        ...
        try:
            ...
            store = get_conversation_store(workspace_root=workspace_root)
    ...
    def _load_tools(self, workspace_root: str, memory_manager, memory_tools: List, session_id: Optional[str] = None, agent_definition=None):
        ...
        for tool_name in tool_manager.tool_classes.keys():
            # Skip tools not in the enabled set (if agent definition restricts tools)
        ...
        if memory_tools:
            ...
            if enabled_tools is not None:
                selected_memory_tools = [
                    ...
                    if getattr(tool, "name", "") in enabled_tools
                ]
            tools.extend(selected_memory_tools)
            if session_id is None:
                logger.info(f"[AgentInitializer] Added {len(selected_memory_tools)} memory tools")
    ...
    def _initialize_skill_manager(self, workspace_root: str, session_id: Optional[str] = None, agent_definition=None):
        """Initialize skill manager"""
        try:
    ...
    def _setup_mcp_tools(self, tools: List, session_id: Optional[str] = None, agent_definition=None) -> Optional[Any]:
        ...
        try:
            ...
            if session_id is None:
                logger.info(
                    f"[AgentInitializer] Loaded {len(all_tools)} MCP tools from "
                    f"{len(mcp_manager.clients)} server(s)"
                )
            return mcp_manager
