...
# in conversation history. Full reasoning may still be streamed to the UI when
# thinking display is enabled; this bound prevents DB/history bloat.
...
class AgentStreamExecutor:
    ...
    def _filter_think_tags(self, text: str) -> str:
        """
        Handle <think>...</think> blocks returned by some providers.

        - When thinking is enabled: remove tags but keep their content.
        - When thinking is disabled: remove the hidden thinking block entirely.
        """
        if not text:
            return text
        import re
    ...
    def run_stream(self, user_message: str) -> str:
        ...
        # Store cancel_token on agent so tools (e.g. MCPTool) can access it
        if self.cancel_token and self.agent:
            self.agent._current_cancel_token = self.cancel_token
        ...
        try:
            while turn < self.max_turns:
                # Cooperative cancellation check at each turn
                ...
                try:
                    for tool_call in tool_calls:
                        # Cooperative cancellation check before each tool
                        if self.cancel_token:
                            self.cancel_token.check_cancelled()
    ...
    def _call_llm_stream(self, retry_on_empty=True, retry_count=0, max_retries=3,
        ...
        """
        # Cooperative cancellation check before LLM call
        if self.cancel_token:
            self.cancel_token.check_cancelled()
        ...
        try:
            ...
            for chunk in stream:
                if isinstance(chunk, dict) and isinstance(chunk.get("usage"), dict) and self.agent:
                    self.agent.last_usage = dict(chunk["usage"])
            ...
            # CancelledError should propagate immediately, not be caught by generic handler
            raise
        except Exception as e:
            error_str = str(e)
        ...
        if full_reasoning:
            ...
            if len(stored_reasoning) < len(full_reasoning):
                logger.info(
                    "[Agent] Truncated stored reasoning: "
    ...
    def _execute_tool(self, tool_call: Dict) -> Dict[str, Any]:
        ...
        # Cooperative cancellation check before tool execution
        if self.cancel_token:
            self.cancel_token.check_cancelled()
