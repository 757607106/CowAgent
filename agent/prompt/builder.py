...
def build_agent_system_prompt(
    ...
    **kwargs
    ...
    if knowledge_enabled is None:
        resolved_knowledge_enabled = bool(conf().get("knowledge", True))
    else:
        resolved_knowledge_enabled = bool(knowledge_enabled)
    if resolved_knowledge_enabled:
...
def _build_memory_section(memory_manager: Any, tools: Optional[List[Any]], language: str) -> List[str]:
    ...
    lines = [
        ...
        "3. search 无结果 → 优先使用已加载的 `MEMORY.md` 摘要；不要仅凭 `MEMORY.md` 的日期摘要构造每日记忆路径",
        "",
        "**记忆文件结构**:",
        "- `MEMORY.md`: 长期记忆索引（已自动加载到上下文，核心信息、偏好、决策等）",
        f"- `memory/YYYY-MM-DD.md`: 每日记忆（懒创建），今天可能是 `memory/{today_file}`；只有工具检索结果或文件列表明确出现时才读取",
