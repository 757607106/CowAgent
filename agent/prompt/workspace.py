...
def ensure_workspace(
    ...
    Args:
        ...
        knowledge_enabled: 是否启用知识库；未指定时回退到全局配置
    ...
    resolved_knowledge_enabled = conf().get("knowledge", True) if knowledge_enabled is None else bool(knowledge_enabled)
    ...
    if create_templates:
        ...
        rule_template = _get_rule_template()
        if not resolved_knowledge_enabled:
            rule_template = _strip_knowledge_sections(rule_template)
        _create_template_if_missing(rule_path, rule_template)
...
def _strip_knowledge_sections(rule_template: str) -> str:
    """Remove knowledge-specific instructions from the default RULE.md template."""
    cleaned = rule_template.replace(
        """├── knowledge/        # 结构化知识库（持续积累的知识）
│   ├── index.md      # 知识目录索引（必须维护）
│   ├── log.md        # 知识操作日志
│   └── <子目录>/     # 按需创建，参考 index.md 已有分类
...
""",
    ...
    cleaned = cleaned.replace("5. **结构化知识 → knowledge/**（见下方知识系统）\n", "")
    cleaned = re.sub(r"\n## 知识系统\n.*?\n## 安全\n", "\n## 安全\n", cleaned, flags=re.S)
    return cleaned
